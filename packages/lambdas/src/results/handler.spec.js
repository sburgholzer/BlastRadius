...
function createMockDeps(options) {
    ...
    const authResolver = {
        resolvePolicy: vitest_1.vi.fn().mockResolvedValue(policy),
    };
    return {
        docClient: docClient,
        s3Client: s3Client,
...
function createStatusItem(overrides) {
    return {
        analysisId: 'analysis-001',
        requestingPrincipal: 'arn:aws:iam::123456789012:user/alice',
        originatingAccountId: '123456789012',
...
function createRequestContext(principalArn, accountId) {
    return {
        identity: {
...
// ─── Tests ───────────────────────────────────────────────────────────────────
(0, vitest_1.describe)('Results Retrieval Handler', () => {
    (0, vitest_1.describe)('input validation', () => {
        (0, vitest_1.it)('rejects null input', async () => {
            const deps = createMockDeps();
            const result = await (0, handler_1.handler)(null, deps);
            (0, vitest_1.expect)(result).toEqual({
                error: 'Invalid input: expected an object with an "operation" field',
                statusCode: 400,
            });
        });
        (0, vitest_1.it)('rejects invalid operation', async () => {
            const deps = createMockDeps();
            const result = await (0, handler_1.handler)({ operation: 'delete' }, deps);
            (0, vitest_1.expect)(result).toEqual({
                error: 'Invalid operation: must be "get" or "list"',
                statusCode: 400,
            });
        });
        (0, vitest_1.it)('rejects request without SigV4 identity', async () => {
            const deps = createMockDeps();
            const input = {
                operation: 'get',
                analysisId: 'analysis-001',
            ...
            (0, vitest_1.expect)(result).toHaveProperty('statusCode', 401);
        });
        (0, vitest_1.it)('rejects get request without analysisId', async () => {
            const deps = createMockDeps();
            const input = {
                operation: 'get',
                analysisId: '',
                requestContext: createRequestContext('arn:aws:iam::123456789012:user/alice', '123456789012'),
            };
            const result = await (0, handler_1.handler)(input, deps);
            (0, vitest_1.expect)(result).toEqual({
    ...
    (0, vitest_1.describe)('get operation - owner access', () => {
        (0, vitest_1.it)('returns full results when the requesting principal is the owner', async () => {
            ...
            const output = result;
            (0, vitest_1.expect)(output.result.analysisId).toBe('analysis-001');
            (0, vitest_1.expect)(output.result.scoredResources).toHaveLength(1);
            (0, vitest_1.expect)(output.exclusionSummary).toEqual({
                excludedAccounts: [],
                excludedRegions: [],
        ...
        (0, vitest_1.it)('returns 404 when analysis does not exist', async () => {
        ...
        (0, vitest_1.it)('returns 404 when result is not yet available (still running)', async () => {
            ...
            const result = await (0, handler_1.handler)(input, deps);
            (0, vitest_1.expect)(result).toHaveProperty('statusCode', 404);
            (0, vitest_1.expect)(result).toHaveProperty('error');
        });
    });
    (0, vitest_1.describe)('get operation - account-based access', () => {
        (0, vitest_1.it)('allows access when principal is authorized for the originating account', async () => {
            const analysisResult = createAnalysisResult({
                requestingPrincipal: 'arn:aws:iam::123456789012:user/alice',
            });
            const statusItem = createStatusItem({
            ...
            const input = {
                operation: 'get',
                analysisId: 'analysis-001',
                requestContext: createRequestContext('arn:aws:iam::987654321098:user/bob', '987654321098'),
            };
            const result = await (0, handler_1.handler)(input, deps);
            (0, vitest_1.expect)(result).toHaveProperty('result');
        });
        (0, vitest_1.it)('denies access when principal is not authorized for the originating account', async () => {
            const statusItem = createStatusItem({
                requestingPrincipal: 'arn:aws:iam::123456789012:user/alice',
                originatingAccountId: '123456789012',
            });
        ...
        (0, vitest_1.it)('scopes results for non-owner access, filtering unauthorized resources', async () => {
            const analysisResult = createAnalysisResult({
                requestingPrincipal: 'arn:aws:iam::123456789012:user/alice',
                scoredResources: [
                    {
            ...
            (0, vitest_1.expect)(output.result.scoredResources).toHaveLength(1);
            (0, vitest_1.expect)(output.result.scoredResources[0].resourceId).toBe('i-12345');
            // Exclusion summary should mention the excluded account
            (0, vitest_1.expect)(output.exclusionSummary.excludedAccounts).toContain('555555555555');
            (0, vitest_1.expect)(output.exclusionSummary.omittedResourceCount).toBe(1);
        });
    });
    (0, vitest_1.describe)('list operation', () => {
        (0, vitest_1.it)('returns results owned by the requesting principal', async () => {
            const statusItem = createStatusItem();
            const deps = createMockDeps({ statusItem });
            const input = {
                operation: 'list',
            ...
            const output = result;
            (0, vitest_1.expect)(output.results).toHaveLength(1);
            (0, vitest_1.expect)(output.results[0].analysisId).toBe('analysis-001');
        });
        (0, vitest_1.it)('rejects list request without SigV4 identity', async () => {
            const deps = createMockDeps();
            const input = {
                operation: 'list',
                requestContext: undefined,
    ...
    (0, vitest_1.describe)('S3 retrieval errors', () => {
        (0, vitest_1.it)('returns 500 when S3 retrieval fails', async () => {
            const statusItem = createStatusItem();
