...
import { scopeDependencyGraph, scopeScoredResources } from './access-scoper';
import type { AuthorizationPolicy } from './access-scoper';
...
function arbitraryNode(
  accountId: fc.Arbitrary<string>,
  region: fc.Arbitrary<string>
): fc.Arbitrary<DependencyNode> {
  return fc.record({
    ...
    dependencyCoverage: fc.constantFrom('full', 'partial', 'unknown') as fc.Arbitrary<'full' | 'partial' | 'unknown'>,
  });
}
 ...
 */
function arbitraryMultiAccountGraph(): fc.Arbitrary<{
  ...
  return fc
    ...
    .filter(
      ({ authorizedAccounts, unauthorizedAccounts }) =>
    ...
    .chain(({ authorizedAccounts, unauthorizedAccounts, authorizedRegions }) => {
      ...
      const authorizedNodesArb = fc.array(
        arbitraryNode(
          fc.constantFrom(...authorizedAccounts),
          fc.constantFrom(...authorizedRegions)
        ),
        { minLength: 1, maxLength: 10 }
      );

      // Generate nodes from unauthorized accounts (using any region)
      const unauthorizedNodesArb = fc.array(
        arbitraryNode(
          fc.constantFrom(...unauthorizedAccounts),
 ...
 */
function arbitraryMultiAccountResources(): fc.Arbitrary<{
  ...
  return fc
    ...
    .chain(({ authorizedAccounts, unauthorizedAccounts, authorizedRegions }) => {
      const authorizedResourcesArb = fc.array(
      ...
      const unauthorizedResourcesArb = fc.array(
        arbitraryScoredResource(
          fc.constantFrom(...unauthorizedAccounts),
          fc.constantFrom(...REGIONS)
        ),
...
describe('Feature: blast-radius-visualizer, Property 18: Access Scoping Excludes Unauthorized Resources', () => {
  /**
   * **Validates: Requirements 9.3, 9.4, 9.5**
   *
   * scopeDependencyGraph results contain zero resources from unauthorized accounts.
   */
  it('scoped dependency graph contains zero nodes from unauthorized accounts', () => {
    fc.assert(
      fc.property(
        arbitraryMultiAccountGraph(),
        ({ graph, authorizedAccounts, authorizedRegions }) => {
          const policy: AuthorizationPolicy = {
            authorizedAccounts,
            authorizedRegions,
          };

          const result = scopeDependencyGraph(graph, policy);

          // Verify: no node in the result belongs to an unauthorized account
          for (const node of result.graph.nodes) {
            expect(authorizedAccounts).toContain(node.accountId);
          }

          // Verify: no node in the result belongs to an unauthorized region
          for (const node of result.graph.nodes) {
            expect(authorizedRegions).toContain(node.region);
          }
        }
      ),
      { numRuns: 100 }
   ...
   */
  it('scoped scored resources contain zero entries from unauthorized accounts', () => {
    fc.assert(
      fc.property(
        arbitraryMultiAccountResources(),
        ({ resources, authorizedAccounts, authorizedRegions }) => {
          const policy: AuthorizationPolicy = {
            authorizedAccounts,
            authorizedRegions,
          };
          ...
          for (const resource of result.resources) {
            expect(authorizedAccounts).toContain(resource.accountId);
          }

          // Verify: no resource in the result belongs to an unauthorized region
          for (const resource of result.resources) {
            expect(authorizedRegions).toContain(resource.region);
          }
        }
      ),
      { numRuns: 100 }
   ...
   */
  it('exclusion summary lists all excluded accounts from dependency graph scoping', () => {
    fc.assert(
      fc.property(
        arbitraryMultiAccountGraph(),
        ({ graph, authorizedAccounts, unauthorizedAccounts, authorizedRegions }) => {
          const policy: AuthorizationPolicy = {
            authorizedAccounts,
            authorizedRegions,
          };
          ...
          const unauthorizedAccountsInGraph = new Set(
            graph.nodes
          ...
          for (const account of unauthorizedAccountsInGraph) {
            expect(result.exclusionSummary.excludedAccounts).toContain(account);
          }

          // The omitted count should match the number of excluded nodes
          const excludedNodeCount = graph.nodes.filter(
            (n) =>
              !authorizedAccounts.includes(n.accountId) ||
              !authorizedRegions.includes(n.region)
          ).length;
          expect(result.exclusionSummary.omittedResourceCount).toBe(excludedNodeCount);
        }
      ),
      { numRuns: 100 }
    );
   ...
   */
  it('exclusion summary lists all excluded accounts from scored resources scoping', () => {
    fc.assert(
      fc.property(
        arbitraryMultiAccountResources(),
        ({ resources, authorizedAccounts, unauthorizedAccounts, authorizedRegions }) => {
          const policy: AuthorizationPolicy = {
            authorizedAccounts,
            authorizedRegions,
          };
          ...
          const unauthorizedAccountsInResources = new Set(
            resources
          ...
          const excludedResourceCount = resources.filter(
            (r) =>
              !authorizedAccounts.includes(r.accountId) ||
              !authorizedRegions.includes(r.region)
          ).length;
          expect(result.exclusionSummary.omittedResourceCount).toBe(excludedResourceCount);
        }
      ),
      { numRuns: 100 }
    );
   ...
   */
  it('scoped graph edges only reference authorized nodes', () => {
    fc.assert(
      fc.property(
        arbitraryMultiAccountGraph(),
        ({ graph, authorizedAccounts, authorizedRegions }) => {
          ...
          for (const edge of result.graph.edges) {
            expect(authorizedNodeIds.has(edge.sourceId)).toBe(true);
            expect(authorizedNodeIds.has(edge.targetId)).toBe(true);
          }
        }
      ),
      { numRuns: 100 }
